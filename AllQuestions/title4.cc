  ...
  static double FindMedianSortedArrays(const std::vector<int> &nums1, const std::vector<int> &nums2) {
    ...
    for (int64_t i = 0; i <= n; ++i) {
      ...
      if (leftPoint >= size1) {  // 即将进行的操作下标将越界
        if (midData == 0)
          midData = (static_cast<double>(nums2[m - leftPoint] + nums2[n - leftPoint])) / 2;
        else
      ...
      if (rightPoint >= size2) {  // 即将进行的操作下标将越界
        if (midData == 0)
          midData = (static_cast<double>(nums1[m - rightPoint] + nums1[n - rightPoint])) / 2;
        else
...
// 性能最优解法
  static int FindKth(const std::vector<int> &nums1, const std::vector<int> &nums2, int k) {
    if (nums1.empty()) return nums2[k - 1];
    if (nums2.empty()) return nums1[k - 1];
    if (k == 1) return std::min(nums1[0], nums2[0]);
    int i = std::min(static_cast<int>(nums1.size()), k / 2), j = std::min(static_cast<int>(nums2.size()), k / 2);
    if (nums1[i - 1] > nums2[j - 1]) {
      return FindKth(nums1, std::vector<int>(nums2.begin() + j, nums2.end()), k - j);
    } else {
      return FindKth(std::vector<int>(nums1.begin() + i, nums1.end()), nums2, k - i);
    }
    return 0;
  }

  static double FindMedianSortedArrays2(const std::vector<int> &nums1, const std::vector<int> &nums2) {
