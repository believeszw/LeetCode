...
//
// 在 Unix 风格的文件系统中，一个点（.）表示当前目录本身；此外，两个点 （..） 表示将目录切换到上一
//
// 级（指向父目录）；两者都可以是复杂相对路径的组成部分。更多信息请参阅：Linux / Unix中的绝对路径 vs 相对路径
//
...
TEST(Title6, test) {
  ...
  std::cout << "SimplifyPath cost time : " << diff(start, end) << " us." << std::endl;
  EXPECT_EQ(str6_2, ret);
